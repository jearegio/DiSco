    ...
    def __call__(self, data_list):
        ...
        for data in data_list:
            ab_g = pgl.Graph(
                    ...
                    edges=data['edges'],
                    node_feat={name: paddle.to_tensor(data[name].reshape([-1, 1])) for name in self.atom_names},
                    edge_feat={name: paddle.to_tensor(data[name].reshape([-1, 1])) for name in self.bond_names + self.bond_float_names})
            ba_g = pgl.Graph(
                    num_nodes=len(data['edges']),
                    edges=data['BondAngleGraph_edges'],
                    node_feat={},
                    edge_feat={name: paddle.to_tensor(data[name].reshape([-1, 1])) for name in self.bond_angle_float_names})
