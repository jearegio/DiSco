...
def sample_different_sizes_and_save(args, device, generative_model,
    ...
    if n_nodes is None:
        ...
        nodesxsample = torch.full((n_samples, 1), n_nodes)
...
def main():
    ...
    sample_different_sizes_and_save(
        args, eval_args, device, flow, nodes_dist,
        dataset_info=dataset_info, n_samples=1) # changed from 30 for testing

    print('Sampling stable molecules.')
    sample_only_stable_different_sizes_and_save(
        args, eval_args, device, flow, nodes_dist,
        dataset_info=dataset_info, n_samples=1, n_tries=2)
